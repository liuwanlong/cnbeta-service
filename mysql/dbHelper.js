...
const saveArticles = (params) => {
  const insertSql = 'INSERT INTO articles(sid,aid,title,inputtime,thumb,label,source,url_show) VALUES ? on duplicate key update title = title';
...
const getArticles = (page, pageSize, callback) => {
  ...
  const sql = 'SELECT pk, sid, aid, title, inputtime, thumb, label FROM articles WHERE content is not NULL ORDER BY inputtime DESC LIMIT ?,?';
